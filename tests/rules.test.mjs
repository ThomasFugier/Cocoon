...
test("chat screen uses the dark ephemeral conversation layout", () => {
  ...
  assert.match(mainApp, /const \[matchRevealDockDark, setMatchRevealDockDark\] = useState\(false\)/);
  ...
  assert.match(mainApp, /const darkTabDock = tab === "chat" \|\| \(tab === "match" && matchRevealDockDark\)/);
  assert.match(mainApp, /const tabDockFadeColors: readonly \[string, string, string\] = darkTabDock[\s\S]{0,140}candy\.darkColor/);
  ...
  assert.match(mainApp, /styles\.tabDock, darkTabDock && styles\.tabDockDark/);
  ...
  assert.match(mainApp, /<ChatScreen[\s\S]{0,120}bottomContentInset=\{visibleBottomContentInset\}/);
  assert.match(mainApp, /<ChatScreen[\s\S]{0,520}onBack=\{\(\) => onTabChange\("home"\)\}/);
  ...
  assert.match(chatScreen, /bottomContentInset: number/);
  ...
  assert.match(chatScreen, /const composerBottomPadding = Math\.max\(bottomContentInset, bottomInteractiveInset, bottomNavInset, appLayout\.bottomInteractiveInset\)/);
...
test("home keeps a static-first responsive fallback", () => {
  ...
  assert.match(app, /const navVisibleHeight = tabDockHeight \+ 32/);
  ...
  assert.match(app, /alignSelf: "stretch",[\s\S]{0,180}width: "100%"/);
  ...
  assert.match(app, /<Entrance delay=\{40\} style=\{styles\.homeSectionEntrance\}>/);
  assert.match(app, /<Entrance delay=\{100\} style=\{styles\.homeSectionEntrance\}>/);
  assert.match(app, /<Entrance delay=\{160\} style=\{styles\.homeSectionEntrance\}>/);
  assert.match(app, /<Entrance delay=\{220\} style=\{styles\.homeSectionEntrance\}>/);
  assert.match(app, /homeSectionEntrance: \{[\s\S]{0,100}alignSelf: "stretch"[\s\S]{0,80}width: "100%"/);
  assert.doesNotMatch(app, /styles\.homeHero,[\s\S]{0,160}aspectRatio: 1\.92/);
  ...
  assert.doesNotMatch(app, /styles\.dailyAdviceCard,[\s\S]{0,180}aspectRatio: 4\.15/);
  assert.doesNotMatch(app, /const packAspectRatio = compact \? 2\.62 : 2\.18/);
  assert.doesNotMatch(app, /styles\.homeStorePackFeatured,[\s\S]{0,180}aspectRatio: packAspectRatio/);
...
test("envies header top gap matches the bottom navbar rhythm", () => {
  ...
  assert.doesNotMatch(app, /enviesStickyHeader: \{[\s\S]{0,180}elevation:/);
...
test("match tab reveals hidden matches first then lists all common matches", () => {
  ...
  assert.match(matchScreen, /const inlineRevealStageHeight = Math\.max\(620, matchLayout\.frameHeight\)/);
  assert.match(matchScreen, /onRevealModeChange\?: \(active: boolean\) => void/);
  assert.match(matchScreen, /onRevealModeChange\?\.\(showInlineReveal\)/);
  assert.match(matchScreen, /return \(\) => onRevealModeChange\?\.\(false\)/);
  ...
  assert.match(matchScreen, /toValue: 0\.58,[\s\S]{0,80}duration: 360/);
  assert.match(matchScreen, /revealAnim\.stopAnimation\(\(\) => \{[\s\S]{0,120}Animated\.spring\(revealAnim/);
  assert.match(matchScreen, /friction: 7,[\s\S]{0,80}tension: 118,[\s\S]{0,80}toValue: 1/);
  ...
  assert.match(app, /function MatchRevealedCardVisual/);
  assert.match(app, /matchRevealTheaterBackdrop/);
  assert.match(app, /matchRevealedPanelInline/);
  assert.match(app, /matchRevealedOrbit/);
  assert.match(app, /matchRevealedHeroBadge/);
  assert.match(app, /matchRevealedCardBackPlate/);
  assert.match(app, /matchRevealedCardBlurb/);
  assert.match(app, /matchScreenRevealMode: \{[\s\S]{0,80}backgroundColor: candy\.darkColor/);
  assert.match(app, /matchPrimaryStageReveal: \{[\s\S]{0,80}backgroundColor: candy\.darkColor/);
...
test("opened match detail uses the dark revealed-match layout", () => {
  ...
  assert.match(matchDetailModal, /Match r.v.l./);
  assert.match(matchDetailModal, /C'est un match/);
  assert.match(matchDetailModal, /Vous avez r.pondu oui, tous les deux/);
  assert.match(matchDetailModal, /<MatchRevealedCardVisual[\s\S]{0,80}activeVoteText=\{activeVoteText\}[\s\S]{0,80}detail[\s\S]{0,80}match=\{match\}/);
  assert.match(app, /styles\.matchRevealedCardShell/);
  assert.match(app, /styles\.matchRevealedBigCard/);
  assert.match(app, /styles\.matchRevealedCategory/);
  assert.match(app, /styles\.matchRevealedAnswerRow/);
  assert.match(app, /<MatchAnswerPill label="Toi" mine value=\{activeVoteText\} \/>/);
  assert.match(app, /<MatchAnswerPill label=\{partnerName\} value=\{partnerVoteText\} \/>/);
  assert.match(app, /matchRevealedStickerSeal/);
  assert.match(app, /matchRevealedMatchBadge/);
...
test("pack selector opens as a full-screen pack grid", () => {
  ...
  assert.match(categoryPicker, /colors=\{\[candy\.red, "rgba\(245,40,110,0\.96\)", "rgba\(245,40,110,0\.5\)", "rgba\(245,40,110,0\)"\]\}/);
  ...
  assert.match(categoryPicker, /const showPartnerPackStatus = Boolean\(pack\.partnerStatusLabel\)/);
  assert.match(app, /function partnerPackOwnershipLabel\(couple: CoupleState, category: DesireCategory\) \{[\s\S]{0,120}Partenaire l'a[\s\S]{0,80}undefined/);
  assert.doesNotMatch(app, /Partenaire ne l'a pas/);
  ...
  assert.match(categoryPicker, /styles\.categoryPickerBadgeRow/);
  assert.match(categoryPicker, /styles\.categoryPickerLockInline/);
  ...
  assert.match(app, /categoryPickerHeaderShell: \{[\s\S]{0,80}zIndex: 30/);
  assert.doesNotMatch(app, /categoryPickerHeaderShell: \{[\s\S]{0,120}elevation:/);
  assert.match(app, /categoryPickerHeader: \{[\s\S]{0,120}backgroundColor: candy\.red[\s\S]{0,160}paddingBottom: 20/);
  assert.match(categoryPicker, /colors=\{\[candy\.red, "rgba\(245,40,110,0\.96\)", "rgba\(245,40,110,0\.5\)", "rgba\(245,40,110,0\)"\]\}/);
  assert.match(app, /categoryPickerHeaderFade: \{[\s\S]{0,80}bottom: -48[\s\S]{0,80}height: 60[\s\S]{0,120}zIndex: 24/);
  ...
  assert.match(app, /categoryPickerBadgeRow: \{[\s\S]{0,160}justifyContent: "space-between"[\s\S]{0,80}width: "100%"/);
  assert.match(app, /categoryPickerLockInline: \{[\s\S]{0,40}marginTop: 0/);
  ...
  assert.match(app, /categoryPickerPartnerTag: \{[\s\S]{0,180}minHeight: 38/);
...
test("store uses the responsive boutique layout", () => {
  ...
  assert.match(storeScreen, /const showPartnerPackStatus = Boolean\(pack\.partnerStatusLabel\)/);
  assert.match(storeScreen, /styles\.storePackMetaRow/);
  assert.match(storeScreen, /styles\.storePackPricePill/);
  assert.match(storeScreen, /styles\.storePackPartnerTag/);
  assert.match(storeScreen, /\{pack\.partnerStatusLabel\}/);
  assert.match(storeScreen, /styles\.storePackCount/);
  assert.match(storeScreen, /\{pack\.countLabel\}/);
  assert.match(storeScreen, /\{pack\.statusLabel\}/);
  assert.match(app, /const pack = packPresentation\(category, couple, \{ included \}\)/);
  assert.match(app, /const showPartnerPackStatus = Boolean\(pack\.partnerStatusLabel\)/);
  ...
  assert.match(app, /storePackMetaRow: \{[\s\S]{0,160}justifyContent: "space-between"[\s\S]{0,80}width: "100%"/);
  assert.match(app, /storePackPricePill: \{[\s\S]{0,140}backgroundColor: "rgba\(38,18,46,0\.9\)"/);
  assert.match(app, /storePackPartnerTag: \{[\s\S]{0,120}backgroundColor: "rgba\(255,249,240,0\.92\)"/);
  assert.match(app, /storePackPartnerTag: \{[\s\S]{0,180}minHeight: 24/);
...
test("gallery card filters default to all cards", () => {
  ...
  assert.match(app, /enviesStickyHeader: \{[\s\S]{0,120}backgroundColor: candy\.red[\s\S]{0,220}zIndex: 40/);
  assert.doesNotMatch(app, /enviesStickyHeader: \{[\s\S]{0,220}elevation:/);
...
test("couple tab uses the compact nous dashboard", () => {
  ...
  assert.match(coupleScreen, /const coupleContentGap = Math\.round\(Math\.min\(22, Math\.max\(14, viewportHeight \* 0\.015\)\)\)/);
  ...
  assert.match(app, /coupleProfileCard: \{[\s\S]{0,180}minHeight: 174/);
  assert.match(app, /coupleProfileAvatar: \{[\s\S]{0,120}height: 56[\s\S]{0,80}width: 56/);
  assert.match(app, /coupleStatPill: \{[\s\S]{0,160}backgroundColor: candy\.black[\s\S]{0,140}minHeight: 88/);
  assert.match(app, /coupleRecentMatch: \{[\s\S]{0,180}minHeight: 58/);
  assert.match(app, /couplePackSummary: \{[\s\S]{0,180}minHeight: 80/);
  assert.match(app, /coupleReconnectCode: \{[\s\S]{0,120}fontSize: 24/);
